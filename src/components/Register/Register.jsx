...
let data =  [
  {'dni': 0123445677, 'name': 'Ramona', 'lastname': 'Pérez Mendiola', 'email': 'rapez08@mymail.com', 'password': 'GT56L98m900X2'},
  {'dni': 2233445566, 'name': 'Joaquin', 'lastname': 'Hermosillo López', 'email': 'joacoh_lopez@mymail.com', 'password': 'igt70HffeE3Hz'},
  {'dni': 12242354209, 'name': 'Luz Ángela', 'lastname': 'Figueroa Arrieta', 'email': 'figueroaluz9@mymail.com', 'password': '45fgkjtOphs48N'},
  {'dni': 2348393095, 'name': 'Lina', 'lastname': 'Garcés Uribe', 'email': 'ligaru33@mymail.com', 'password': '327h67&7G679975o'}
];
    ...
    <div className="flex min-h-screen bg-gray-200">
      <div className="grid grid-rows-6 w-screen divide-y-4 divide-solid divide-gray-300 h-96 visible">
        ...
        <div className="row-span-5 h-full py-12">
          <form className="grid grid-rows-2 gap-1 bg-white w-5/6 h-auto shadow-md shadow-slate-400 mx-5 xs:p-20 sm:mx-14 md:mx-20 lg:mx-24" onSubmit={submitHandler}>
            ...
            <ul className="grid grid-rows-5 grid-cols-3 gap-2 mx-3 my-0 sm:grid-rows-5 grid-cols-2 md:grid-rows-3 grid-cols-3 lg:grid-rows-3 grid-cols-3 xl:grid-rows-3 grid-cols-3 2xl:grid-rows-3 grid-cols-3">
              <li className="flex flex-col">
                ...
                <input type="text" id="fullnameLogup" className="bg-custom-st-patrick-yw h-8 rounded-md w-18 sm:w-38 md:w-56" value={name} onChange={(ev) => { console.log(name); setName(ev.target.value); }} />
              </li>
              <li className="flex flex-col">
                <label className="mr-2 px-1">Apellidos completos</label>
                <input type="text" className="bg-custom-st-patrick-yw h-8 rounded-md w-18 sm:w-38 md:w-56" onChange={(ev) => { console.log(lastname); setLastname(ev.target.value); }} />
              </li>
              <li className=" flex flex-col">
                <label className="mx-2 px-1">Número de Identificación</label>
                <input type="text" className="bg-custom-st-patrick-yw h-8 rounded-md w-18 sm:w-38 md:w-56" onChange={(ev) => { console.log(dni); setDni(ev.target.value); }} />
              </li>
              <li className=" flex flex-col">
                <label className="mx-2 px-1">Teléfono</label>
                <input type="text" className="bg-custom-st-patrick-yw h-8 rounded-md w-18 sm:w-38 md:w-56" onChange={(ev) => { console.log(phone); setPhone(ev.target.value); }} />
              </li>
              <li className=" flex flex-col">
                <label className="mx-2 px-1">Número de producto</label>
                <input type="text" className="bg-custom-st-patrick-yw h-8 rounded-md w-18 sm:w-38 md:w-56" onChange={(ev) => { console.log(productNm); setProductNm(ev.target.value); }} />
              </li>
              <li className=" flex flex-col">
                <label className="mx-2 px-1">Correo Electrónico</label>
                <input type="text" className="bg-custom-st-patrick-yw h-8 rounded-md w-18 sm:w-38 md:w-56" onChange={(ev) => { console.log(email); setEmail(ev.target.value); }} />
              </li>

              <li className=" flex flex-col xs:bg-red-100">
                <label>Crear contraseña</label>
                <input type="text" className="bg-custom-st-patrick-yw h-8 rounded-md w-18 sm:w-38 md:w-56" onChange={(ev) => { console.log(password); setPassword(ev.target.value); }} />
              </li>
