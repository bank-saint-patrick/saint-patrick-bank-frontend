...
export default function Transaction(props) {
    const { id, type, receptor, timestamp, ammount, sender, img, concepto } = props;

    const textColor = type === 'Ingreso' ? 'text-green-500' : 'text-red-500';
    const iconType = type === 'Ingreso' ? <FontAwesomeIcon className="text-green-600" icon={faArrowDown} /> : <FontAwesomeIcon className="text-red-600" icon={faArrowUp} />;
        ...
        <section className="flex flex-col md:flex-row w-full justify-between items-center border-b-2 border-gray-300">
            <div className="w-full md:w-1/3 grow flex justify-start py-4 whitespace-nowrap">
                <div className="flex items-center flex-col xsm:flex-row justify-between w-full md:justify-start">
                    <div className="mr-6 relative">
                        ...
                        <FontAwesomeIcon className={`${type === 'Ingreso' ? 'text-green-500' : 'text-red-500'} absolute right-0 bottom-0`} icon={faCircle} />
                    </div>
            ...
            <div className="hidden w-1/3 md:flex justify-center py-4 whitespace-nowrap">
                <div className="text-left text-base font-semibold">
                    # {id < 10 ? `0${id}` : id} - {concepto.substring(0, 25) + (concepto.length > 25 ? '...' : '')}
                </div>
            </div>
            ...
            <div className="w-full md:w-1/3 justify-start flex md:justify-end py-4 whitespace-nowrap">
                <div className={`font-bold flex flex-col ${textColor}`}>
                    <span className="font-normal text-lg">
                        <b className="text-xl">{type === 'Ingreso' ? '+' : '-'}</b> ${ammount}
                    </span>
